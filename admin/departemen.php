...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if (is_numeric($deleteId)) {
        $stmt = $connect->prepare("DELETE FROM departemen WHERE id_departemen = ?");
        $stmt->bind_param("i", $deleteId);

        if ($stmt->execute()) {
            echo "<script>alert('Data berhasil dihapus!');</script>";
            echo "<script>window.location.href='departemen.php';</script>"; 
        } else {
...
if (isset($_POST['update'])) {
    ...
    if (!empty($nama_departemen)) {
        $stmt = $connect->prepare("UPDATE departemen SET nama_departemen = ? WHERE id_departemen = ?");
        $stmt->bind_param("si", $nama_departemen, $id_departemen);
        if ($stmt->execute()) {
    ...
    <main class="flex-1 w-full md:w-[calc(100%-256px)] md:ml-64 bg-gray-50 min-h-screen transition-all main">
        <div class="py-2 px-6 bg-white flex items-center shadow-md sticky top-0 left-0 z-30">
            ...
            <div id="breadcrumb" class="flex items-center text-sm ml-4">
                ...
                <span id="breadcrumb-item" class="text-gray-400">Departemen</span>
            </div>
                        ...
                        <table class="min-w-full leading-normal">
                            ...
                            <tbody>
                                ...
                                ?>
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <div class="text-center space-x-3">
                                                <button
                                                    ...
                                                    data-id="editModal<?php echo $user['id_departemen']; ?>">
                                                    Edit
                                                </button>
                                                <a 
                                                    ...
                                                    href="departemen.php?action=delete&id=<?php echo $user['id_departemen']?>"
                                    ...
                                    <!-- modal Edit -->
                                    <div id="editModal<?php echo $user['id_departemen']; ?>"  class="modal fixed inset-0 z-50 hidden">
                                            ...
                                            <div class="modalContent relative w-full max-w-[50rem] h-full max-h-[25rem] transform opacity-0 translate-y-[-100%] bg-gray-100 rounded-xl shadow-2xl transition-all duration-500 ease-out">
                                                ...
                                                <div class="flex-auto px-4 lg:px-6 py-8 pt-0">
                                                    <form action="" method="POST">
                                                        <input type="hidden" name="id_departemen" value="<?php echo $user['id_departemen']; ?>">
                                                        <div class="mb-4">
                                                            <label class="block text-sm font-bold mb-2">Nama Departemen</label>
                                                            <input type="text" name="nama_departemen" value="<?php echo $user['nama_departemen']; ?>" class="border-2 border-gray-300 px-3 py-3 text-gray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full">
                                                        </div>
                ...
                <div id="modalContent" class="relative w-full max-w-[50rem] h-full max-h-[25rem] transform opacity-0 translate-y-[-100%] bg-gray-100 rounded-xl shadow-2xl transition-all duration-500 ease-out">
                    ...
                    <div class="flex-auto px-4 lg:px-6 py-8 pt-0 overflow-y-auto max-h-[30rem]">
                        <form action="" method="POST" enctype="multipart/form-data">
                            <h6 class="text-blueGray-400 text-sm mt-3 mb-4 font-bold uppercase">Nama Departemen</h6>
                            <div class="w-full lg:w-12/12 px-4">
                                <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2">Nama Departemen</label>
                                <input name="nama_departemen" class="border-2 border-gray-300 px-3 py-3 text-blueGray-600 appearance-none bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" required>
                            </div>
                            <!-- Submit Button -->
